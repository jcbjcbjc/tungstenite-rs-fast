...
pub fn connect_with_config<'a,Req: IntoClientRequest>(
    request: Req,
    config: Option<WebSocketConfig>,
    max_redirects: u8,
) -> Result<(WebSocket<'a,MaybeTlsStream<TcpStream>>, Response)> {
    fn try_client_handshake<'a>(
        request: Request,
        config: Option<WebSocketConfig>,
    ) -> Result<(WebSocket<'a,MaybeTlsStream<TcpStream>>, Response)> {
        let uri = request.uri();
...
/// you want to use other TLS libraries, use `client` instead. There is no need to enable any of
/// the `*-tls` features if you don't call `connect` since it's the only function that uses them.
pub fn connect<'a,Req: IntoClientRequest>(
    request: Req,
) -> Result<(WebSocket<'a,MaybeTlsStream<TcpStream>>, Response)> {
...
pub fn client_with_config<'a,Stream, Req>(
    request: Req,
    stream: Stream,
    config: Option<WebSocketConfig>,
) -> StdResult<(WebSocket<'a,Stream>, Response), HandshakeError<ClientHandshake<Stream>>>
where
    Stream: Read + Write,
